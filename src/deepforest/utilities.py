...
def image_to_geo_coordinates(gdf, root_dir=None, flip_y_axis=False):
    ...
    if geom_type == "box":
        ...
        if len(x_coords) != len(y_coords):
            raise ValueError(
                f"Mismatched coordinate lengths: x_coords has {len(x_coords)} elements, "
                f"y_coords has {len(y_coords)} elements"
            )
...
def boxes_to_shapefile(df, root_dir, projected=True, flip_y_axis=False):
    ...
    if projected:
        ...
        # Ensure all coordinate arrays have the same length
        coord_lengths = [
            len(xmin_coords),
            len(ymin_coords),
            len(xmax_coords),
        ...
        if not all(length == coord_lengths[0] for length in coord_lengths):
            raise ValueError(
                f"Mismatched coordinate array lengths: xmin={len(xmin_coords)}, "
                f"ymin={len(ymin_coords)}, xmax={len(xmax_coords)}, ymax={len(ymax_coords)}"
            )
