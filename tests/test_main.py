...
def test_predict_tile_verbose_false(m, raster_path, capsys):
    ...
    m.config["train"]["fast_dev_run"] = False
    m.create_trainer()
    prediction = m.predict_tile(raster_path=raster_path, patch_size=300, patch_overlap=0, in_memory=True, verbose=False)
    assert isinstance(prediction, pd.DataFrame)
    assert not prediction.empty
